...
Ключевое слово <b>using</b>. Указываю для справки, тема очень большая и требует отдельного разбора. Похоже на менеджер конткеста with в python, который автоматически закрывает соединения, удаляет дескрипторы и т.д., когда в них больше нет необходимости.
